...
def evolve_code_node(state, llm, code_llm):
    ...
    new_state = copy.deepcopy(state)
    new_state["current_loop"] = (state.get("current_loop") or 0) + 1
    ...
    current_generation = state.get("current_generation") or 0
    generations = new_state.get("generations") or []
    generation_entry = {
        "generation": current_generation,
        "solutions_list": copy.deepcopy(state.get("solutions_list") or []),
    }
    generations = list(generations)  # ensure mutable list
    ...
    seed_solutions.sort(key=_sort_key, reverse=True)
...
def test_code_node(state: AgentState, llm, code_llm) -> AgentState:
    ...
    assembled_solutions: List[Dict[str, Any]] = []

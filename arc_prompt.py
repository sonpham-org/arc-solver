...
def run_code_repair_attempt(task_id, task_data, model, provider, previous_json, execution_errors, print_input=False, 
    ...
    for i, example in enumerate(training_examples):
        ...
        try:
            ...
            if predicted_output is None:
                print(f"{RED}✗ Repaired code still fails on example {i+1}: execution returned None{RESET}")
                all_successful = False
                continue
            
            if not is_valid_prediction(predicted_output):
                print(f"{RED}✗ Repaired code still fails on example {i+1}: invalid output format{RESET}")
                all_successful = False
                continue
            
            if predicted_output != expected_output:
                overlap = calculate_grid_overlap(predicted_output, expected_output)
                print(f"{RED}✗ Repaired code still fails on example {i+1}: {overlap:.1f}% overlap{RESET}")
                all_successful = False
                continue
            
            print(f"{GREEN}✓ Repaired code works on example {i+1}{RESET}")
            
        except Exception as e:
            print(f"{RED}✗ Repaired code still fails on example {i+1}: {str(e)}{RESET}")
            all_successful = False
    
    if all_successful:
        print(f"{GREEN}🎉 Code repair successful! All training examples pass.{RESET}")
    else:
...
def run_task_with_reflection(task_id, task_data, solutions, model, provider, previous_result, output_dir=None, 
    ...
    if is_valid:
        print(f"2. Valid Structure: {GREEN}✓ YES{RESET}")
    else:
        print(f"2. Valid Structure: {RED}✗ NO{RESET}")
    ...
    if is_valid:
        ...
        python_code = extracted_json.get("python_code", [])
        if python_code:
            print(f"4. Python Code: {GREEN}✓ YES ({len(python_code)} lines){RESET}")
        else:
            print(f"4. Python Code: {RED}✗ NO{RESET}")
        
        # 5. Helper functions?
        helper_functions = extracted_json.get("helper_python_functions", [])
        if helper_functions:
            print(f"5. Helper Functions: {GREEN}✓ YES ({len(helper_functions)} functions){RESET}")
        else:
        ...
        if python_code:
            ...
            if training_examples:
                ...
                if execution_errors:
                    ...
                    print(f"   Execution: {RED}✗ {success_count}/{total_examples} examples passed{RESET}")
                    ...
                    for repair_attempt in range(max_repair_attempts):
                        ...
                        if repair_success and repaired_json:
                            print(f"{GREEN}🎉 Reflection code repair successful! Using repaired JSON.{RESET}")
                    ...
                    total_examples = len(training_examples)
                    print(f"   Execution: {GREEN}✓ {success_count}/{total_examples} examples passed{RESET}")
            else:
                print(f"   {RED}No training examples to test{RESET}")
        else:
            print(f"6. Code Execution: {RED}✗ No Python code to test{RESET}")
    else:
...
def run_task_with_retries(task_id, task_data, solutions, model, provider, output_dir=None, 
    ...
    for attempt in range(task_retries + 1):
        ...
        if result.get('transformations_json_generated', False):
            if attempt > 0:
                print(f"\n{GREEN}✓ SUCCESS: Task {task_id} succeeded on retry attempt {attempt}{RESET}")
            return result
        
        # If this was the last attempt, return the failed result
        if attempt == task_retries:
            if task_retries > 0:
                print(f"\n{RED}✗ FAILED: Task {task_id} failed all {task_retries + 1} attempts to generate valid JSON{RESET}")
            return result
        
        # If we have more attempts, prepare for retry
        print(f"\n{RED}✗ ATTEMPT {attempt + 1} FAILED: Task {task_id} did not generate valid JSON{RESET}")
        if attempt < task_retries:
                ...
                if success_count < total_examples and is_valid:
                    ...
                    if execution_errors:
                        ...
                        for repair_attempt in range(max_repair_attempts):
                            ...
                            if repair_success and repaired_json:
                                print(f"{GREEN}🎉 Code repair successful! Using repaired JSON.{RESET}")
                                extracted_json = repaired_json
                                result['transformations_json'] = extracted_json
                                repair_successful = True
                                break
...
def call_model(prompt, model, provider=None, temperature=None):
    ...
    if temperature is not None and temperature != TEMPERATURE:
        print(f"🔧 Using reflection temperature: {temperature} (instead of main temperature: {TEMPERATURE})")
    ...
    if parallel and len(selected_tasks) > 1:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_task):
                ...
                print(f"\n{BLUE}⏱️ Task {completed_count}/{len(selected_tasks)} completed after {elapsed:.1f}s, processing result...{RESET}")
                sys.stdout.flush()
                sys.stderr.flush()
        ...
        for i, (task_id, task_data) in enumerate(selected_tasks):
            ...
            if is_task_completed_successfully(result):
                completed_tasks.add(task_id)
                print(f"{GREEN}✓ Task {task_id} completed successfully on initial attempt!{RESET}")
            else:
                print(f"{RED}✗ Task {task_id} failed on initial attempt. Will retry with reflection.{RESET}")
        ...
        while failed_tasks and task_runs_used < total_task_runs:
            ...
            for i, (task_id, task_data) in enumerate(current_failed_tasks):
                ...
                if result:
                    ...
                    if is_task_completed_successfully(result):
                        completed_tasks.add(task_id)
                        print(f"{GREEN}✓ Task {task_id} completed successfully after reflection round {reflection_round}!{RESET}")
                    else:
                        failed_tasks.append((task_id, task_data))  # Still failed, try again next round
                        print(f"{RED}✗ Task {task_id} still failed after reflection round {reflection_round}.{RESET}")
                    ...
                    print(f"{RED}✗ Failed to get result for task {task_id} in reflection round {reflection_round}{RESET}")
